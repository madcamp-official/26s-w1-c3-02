...
- 사이드바 항목: 대시보드, 내 주석, 즐겨찾기(즐겨찾기 주석), 내 서재(즐겨찾기 책), 그룹 주석방, 친구, 설정
  - **대시보드**: 프로필 요약(아바타·닉네임·이메일·가입일) + 항목별 개수 통계 카드(내 주석/즐겨찾기/내 서재/그룹 주석방/친구)
  - **설정**: 닉네임 변경 폼 + 로그아웃. 별도 모달 없이 탭 안에 인라인으로 둔다.
- 사이드바 활성 항목은 `--color-primary-soft` 배경 + `--color-primary` 텍스트로 표시한다(7절 네비게이션 활성 표시 규칙과 동일한 톤).
- 반복 목록(내 주석, 즐겨찾기, 내 서재, 그룹 주석방)은 동일한 카드 컴포넌트를 재사용한다.
