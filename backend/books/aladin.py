...
BOOK_SEARCH_TARGETS = ('Book', 'Foreign')


def search_aladin_books(keyword, field=None, size=10, page=1):
    max_results = min(max(int(size or 10), 1), 50)
    params = {
        'Query': keyword,
    ...
    results = []
    seen = set()
    for target in BOOK_SEARCH_TARGETS:
        payload = call_aladin('ItemSearch.aspx', {**params, 'SearchTarget': target})
        for item in payload.get('item', []):
            normalized = normalize_aladin_item(item)
            key = normalized.get('isbn') or normalized.get('aladinItemId')
            if key in seen:
                continue
            seen.add(key)
